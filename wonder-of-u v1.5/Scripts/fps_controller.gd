...
func _on_item_entered(body):
	...
	# Ensure we're detecting an actual item (ignoring child nodes)
	if not body.is_in_group("Item"):
		if body.get_parent() and body.get_parent().is_in_group("Item"):
			body = body.get_parent()
		else:
			print("[DEBUG] Ignoring non-item:", body.name)
			return

	# Check distance and print actual values
	...
	print("[DEBUG] Player Position:", player_pos)
	print("[DEBUG] Item Position:", item_pos)
	print("[DEBUG] Calculated Distance:", distance)

	# Adjust detection range if needed
	if distance > 3.5:  # Try increasing this if needed
		print("[DEBUG] Ignoring", body.name, "because it's too far away (", distance, "m)")
		return

	print("[DEBUG] Detected an item:", body.name)

	highlighted_item = body
	_set_item_highlight(body, true)

	# Wait until the item is fully loaded before spawning the highlight
	call_deferred("_spawn_highlight_sprite", body)
...
func _on_item_exited(body):
	...
	# Ensure we are checking the correct item (if it has a parent that is an item)
	if not body.is_in_group("Item"):
		if body.get_parent() and body.get_parent().is_in_group("Item"):
			body = body.get_parent()
		else:
	...
	# Remove highlight from this specific item
	_set_item_highlight(body, false)
	_remove_highlight_sprite(body)

	# If the exited item was the last highlighted one, reset it
...
func _spawn_highlight_sprite(item):
	if not item.is_inside_tree():
		print("[DEBUG] Item not yet inside tree, skipping highlight sprite.")
		return
	
	print("[DEBUG] Spawning highlight sprite for:", item.name)

	# Check if an existing highlight sprite is already attached
	if item.has_node("HighlightSprite"):
		print("[DEBUG] Highlight already exists on", item.name)
		return

	# Create and attach the highlight sprite
	var highlight_sprite = Sprite3D.new()
	highlight_sprite.name = "HighlightSprite"
	highlight_sprite.texture = preload("res://Sprites/EquipIcon.png")  # Replace with correct path
	highlight_sprite.billboard = BaseMaterial3D.BILLBOARD_ENABLED
	highlight_sprite.pixel_size = 0.008
	highlight_sprite.position = Vector3(0, 1.5, 0)  # Adjust position if needed
